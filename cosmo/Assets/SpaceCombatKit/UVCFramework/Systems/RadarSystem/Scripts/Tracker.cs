            ...
            if (lineOfSight)
            {
                ...
                for (int i = 0; i < hits.Length; ++i)
                {
                    if (hits[i].collider.attachedRigidbody != null)
                    {
                        ...
                        {
                            float distanceToTarget = Vector3.Distance(target.transform.position, referenceTransform.position);
                            float distanceToHit = Vector3.Distance(target.transform.position, referenceTransform.position);
                    ...
                    else
                    {
                        if (debugLineOfSightBlocking)
                        {
                            Debug.DrawLine(referenceTransform == null ? transform.position : referenceTransform.position, hits[i].point, Color.red);
                            float distanceToTarget = Vector3.Distance(target.transform.position, referenceTransform.position);
                            float distanceToHit = Vector3.Distance(target.transform.position, referenceTransform.position);
                            Debug.LogWarning("Line of sight from " + name + " tracker to " + target.name + " target is blocked by " + hits[i].collider.name +
